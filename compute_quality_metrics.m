...
date = '01_09_21';
groupings = {'five-sess'};
versions = {'TYPED_FITHRF_GLMDENOISE_RR'};
% 'TYPEB_FITHRF','TYPEC_FITHRF_GLMDENOISE','TYPED_FITHRF_GLMDENOISE_RR'};%
subjs = {'CSI4'};
nrunimgs = 37;
overwrite = 1;
    ...
    for v = 1:length(versions)
        ...
        for s = 1:length(subjs)
            ...
            if ~isfile(repbeta_savefn) || overwrite == 1
                ...
                disp('done loading design info')
                disp('length of allses_design:')
                disp(length(allses_design))
                disp('length of labels:')
                disp(length(labels))
                ...
                for i = 1:length(ses_subdirs)
                    ...
                    disp('loading data from session:')
                    disp(ses_subdirs{i})
                ...
                for jj = 1:subdims(2)
                    for kk = 1:subdims(3)
                        ...
                        if sum(isnan(aa(:))) == 0
                            ...
                            elseif numreps == 3
                                bb0 = corr(mean(aa([1 2],:))', aa(3,:)'); 
                                bb1 = corr(mean(aa([1 3],:))', aa(2,:)'); 
                                bb2 = corr(mean(aa([2 3],:))', aa(1,:)'); 
                            end
