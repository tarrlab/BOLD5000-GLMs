...
if contains(betadir,'assume')
   out_version = 'TYPEA_ASSUMEHRF';
else
   out_version = version;
end
...
savedir = fullfile(betadir, 'metrics', out_version);
...
for i = 1:size(subdirs,1)
    ...
    if contains(fn,'session')
        ...
        ses = strsplit(fn,'_');
        temp = cell2mat(cellfun(@str2num,ses(2:end),'UniformOutput',0));
        session_groups = [session_groups; temp];
    end
